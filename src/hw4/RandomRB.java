...
public class RandomRB {
	...
	public class Node {
		...
		public Node(int key, boolean color, int red) {
			this.key = key;
			this.color = color;
			this.redcount = red;
		}
	...
	private Node bstInsert(Node x, int key, boolean color) {
		if (x == null) return new Node(key, color, 0);
	...
	public LLRB resetRandTree(int seed, int begin, int inc) {
		...
		if (depth == 0) {
			...
			while (!nodes.isEmpty()) {
				...
				int r = n.redcount;
				if (three) {
					if (r + 1 > reddepth) reddepth = r + 1;
					n.left = new Node(n.key, RED, r + 1);
					if (n.key > 0) {
						n.left.left = new Node(n.key - 1, BLACK, r + 1);
						n.left.right = new Node(n.key - 1, BLACK, r + 1);
						n.right = new Node(n.key - 1, BLACK, r);
						nodes.add(n.left.left);
						nodes.add(n.left.right);
						nodes.add(n.right);
					}
				} else if (n.key > 0) {
					n.left = new Node(n.key - 1, BLACK, r);
					n.right = new Node(n.key - 1, BLACK, r);
					nodes.add(n.left);
					nodes.add(n.right);
				}
